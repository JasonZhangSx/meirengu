...
import com.meirengu.admin.dao.*;
import com.meirengu.admin.model.Type;
import com.meirengu.admin.service.IItemService;
import com.meirengu.admin.util.FileUtil;
import com.meirengu.admin.util.ResultUtil;
import com.meirengu.admin.util.UtilData;
import com.meirengu.admin.vo.*;
...
public class ItemServiceImpl implements IItemService {
     ...
     */
    @Override
    public Map addItemData(ItemVo itemVo, Iterator<String> iter, MultipartHttpServletRequest multipartHttpServletRequest) {
...
//            Validator.getInstance().validate(itemVo);
     ...
     */
    @Override
    public Map getItemData(ItemVo itemVo) {
     ...
     */
    @Override
    public Map updateItemData(ItemVo itemVo) {
     ...
     */
    @Override
    public Map deleteItemData(Integer itemId) {
     ...
     */
    @Override
    public Map getItemDetail(ItemVo itemVo) {
    ...
    @Override
    public Map addItem() {
        List<HospitalVo> hospitalVoList = hospitalDao.conditionQuery(new HospitalVo());
        List<DoctorVo> doctorVoList = doctorDao.conditionQuery(new DoctorVo());
        List<ItemClassVo> itemClassVoList = tree(itemClassDao.getItemClassData(new ItemClassVo()),0);
        List<BrandVo> brandVoList = brandDao.selectByPrimaryKey(new BrandVo());
        List<TypeVo> typeVoList = typeDao.getTypeData(new TypeVo());
        Map<String,Object> map = new HashMap<>();
        map.put("hospital",hospitalVoList);
        map.put("doctor",doctorVoList);
        map.put("itemClass",itemClassVoList);
        map.put("brand",brandVoList);
        map.put("type",typeVoList);
        return map;
    }
