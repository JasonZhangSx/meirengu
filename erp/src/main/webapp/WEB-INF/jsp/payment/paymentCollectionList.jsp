...
<section class="Hui-article-box" style="top: 0; left: 0">
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 财务管理 <span class="c-gray en">&gt;</span> 待收款记录 <a
            class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
            href="javascript:location.replace(location.href);" title="刷新"><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="Hui-article">
        <article class="cl pd-20">
            <form action="getPaymentCommit" method="get">
                <div class="text-c">
            ...
            <div class="mt-20">
                <table class="table table-border table-bordered table-bg table-hover table-sort">
                    ...
                    <c:forEach items="${paymentCollectionList}" var="paymentCollection">
                        <tr class="text-c">
                            <td>${paymentCollection.id}</td>
                            <td>${paymentCollection.partnerName}</td>
                            <td>${paymentCollection.itemName}</td>
                            <td>${paymentCollection.targetAmount}</td>
                            <td>${paymentCollection.completedAmount}</td>
                            <td>${paymentCollection.collectionPeriod}</td>
                            <td>${paymentCollection.receivedPeriod}</td>
                            <td>${paymentCollection.surplusPeriod}</td>
                            <c:choose>
                                <c:when test="${paymentCollection.status==1}">
                                    <td>完成</td>
                                    <td>
                                        <a style="text-decoration:none" class="ml-5" href="javascript:;" onClick="project_edit('财务-待收款列表-收款','getPaymentCollectionList',${paymentCollection.id})" title="收款">查看</a>
                                    </td>
                                </c:when>
                                <c:when test="${paymentCollection.status==0}">
                                    <td>未完成</td>
                                    <td>
                                        <a style="text-decoration:none" class="ml-5" href="javascript:;" onClick="project_edit('财务-待收款列表-收款','getPaymentCollectionList',${paymentCollection.id})" title="收款">收款</a>
                                    </td>
                                </c:when>
                            </c:choose>
                        </tr>
...
//        "aaSorting": [[0, "asc"]],
    ...
    function project_edit(title, url, id, w, h) {
        var index = layer.open({
            type: 2,
            title: title,
            content: url+'?paymentCollectionId='+id
        });
        layer.full(index);
    }
    // 时间
